<template>
  <div class="page-wrap">
    <h2 class="page-wrap__title">项目管理</h2>

    <form class="add-form" @submit.prevent="handleAdd">
      <input v-model="newName" type="text" class="field__input" placeholder="项目名称" />
      <input v-model="newDesc" type="text" class="field__input" placeholder="项目描述" />
      <input v-model="newUrl" type="text" class="field__input" placeholder="链接 URL" />
      <button type="submit" class="btn btn--primary" :disabled="!newName || !newDesc || !newUrl">添加</button>
    </form>
    ...
    <div v-if="projects.length" class="proj-list">
      <div v-for="p in projects" :key="p.id" class="proj-card">
        <div class="proj-card__body">
          ...
          <span class="proj-card__desc">{{ p.desc }}</span>
          <a :href="p.url" target="_blank" class="proj-card__url">{{ p.url }}</a>
        </div>
        <div class="proj-card__actions">
          <button class="btn btn--sm" @click="startEdit(p)">编辑</button>
    ...
    <div v-if="editing" class="modal-mask" @click.self="editing = null">
      <div class="modal">
        ...
        <label class="field">
          ...
          <input v-model="editing.desc" type="text" class="field__input" />
        </label>
        <label class="field">
          <span class="field__label">链接</span>
...
function handleAdd() {
  addProject({ name: newName.value, desc: newDesc.value, url: newUrl.value }, () => {
    newName.value = '';
    newDesc.value = '';
    newUrl.value = '';
    load();
...
function startEdit(p) {
  editing.value = reactive({ id: p.id, name: p.name, desc: p.desc, url: p.url });
}

function handleEdit() {
  updateProject(editing.value.id, {
    name: editing.value.name, desc: editing.value.desc, url: editing.value.url,
...
.proj-card__body { display: flex; flex-direction: column; gap: 2px; min-width: 0; }
.proj-card__name { font-family: var(--font-ink); font-size: 15px; font-weight: 500; color: #3a2f28; }
.proj-card__desc { font-family: var(--font-ink); font-size: 12px; color: rgba(58, 47, 40, 0.45); }
.proj-card__url {
  font-family: var(--font-ink); font-size: 11px; color: rgba(58, 47, 40, 0.35);
  overflow: hidden; text-overflow: ellipsis; white-space: nowrap;
}
.proj-card__actions { display: flex; gap: 6px; flex-shrink: 0; }
